 ...
 */
export function sampleRoute(coordinates, intervalMeters = 500) {
    if (!coordinates || !Array.isArray(coordinates) || coordinates.length < 2) {
        throw new Error('Route must have at least 2 points');
    }
    // Validate and filter coordinates
    const validCoordinates = coordinates.filter(c => Array.isArray(c) &&
        c.length >= 2 &&
        !isNaN(Number(c[0])) &&
        !isNaN(Number(c[1])));
 ...
 */
export function getTotalDistance(coordinates) {
    let total = 0;
 ...
 */
export function sampleRouteByCount(coordinates, targetSamples = 500) {
    if (!coordinates || !Array.isArray(coordinates) || coordinates.length < 2) {
        throw new Error('Route must have at least 2 points');
    }
