...
import { haversineDistance, sampleRoute, sampleRouteByCount, getTotalDistance } from './route-sampler.js';
    ...
    describe('sampleRoute', () => {
        ...
        test('handles multi-segment routes', () => {
            ...
            for (let i = 1; i < sampled.length; i++) {
                assert.ok(sampled[i].distance >= sampled[i - 1].distance, `Distance at index ${i} should be >= previous`);
            }
        });
        ...
        test('distance property increases correctly', () => {
            ...
            for (let i = 1; i < sampled.length; i++) {
                assert.ok(sampled[i].distance > sampled[i - 1].distance, 'Each point should have greater distance than previous');
            }
            ...
            assert.ok(Math.abs(lastDist - totalDist) < 100, 'Last point distance should be close to total distance');
        });
    });
    ...
    describe('sampleRouteByCount', () => {
        ...
        test('points are evenly spaced', () => {
            ...
            for (let i = 1; i < sampled.length; i++) {
                ...
                assert.ok(Math.abs(actualInterval - expectedInterval) < tolerance, `Interval ${i} should be approximately ${expectedInterval}m, got ${actualInterval}m`);
            }
        });
        test('handles multi-segment routes', () => {
            const route = [
...
//# sourceMappingURL=route-sampler.test.js.map