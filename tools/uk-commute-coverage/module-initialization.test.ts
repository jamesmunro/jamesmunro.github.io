...
describe('CoverageAnalyzer module initialization', () => {
  test('module can be imported before Google Maps API is loaded', async () => {
    ...
    const originalGoogle = hasGoogle ? (global as { google: unknown }).google : undefined;
    ...
    try {
      ...
      if (error instanceof ReferenceError && error.message.includes('google')) {
        assert.fail(
          'Module references google.maps at module scope. ' +
      ...
      if (hasGoogle && originalGoogle !== undefined) {
        (global as { google: unknown }).google = originalGoogle;
  ...
  test('module-level constants do not reference google.maps', async () => {
    ...
    await assert.doesNotReject(
      () => import(resolvedPath + '?t=' + Date.now()),
      ReferenceError,
      'Module constants should not reference google.maps at load time'
    );
  });
});
