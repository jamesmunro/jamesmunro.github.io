...
export class GoogleMap {
  ...
  private overlays: google.maps.GroundOverlay[];

  constructor(containerId: string, logger: Logger = console) {
   ...
   */
  addTileOverlay(url: string, bounds: Wgs84Bounds, opacity = 0.5): google.maps.GroundOverlay | undefined {
    if (!this.map) return;
  ...
  drawRoute(coordinates: Array<[number, number]>): void {
    ...
    const path: MapLatLng[] = coordinates
      .map((coord, idx) => {
  ...
  setDirections(directionsResult: google.maps.DirectionsResult): void {
    ...
    if (firstRoute && firstRoute.legs && firstRoute.legs[0]) {
      ...
      const getLatLng = (loc: google.maps.LatLng | null | undefined): MapLatLng => {
        if (!loc) return { lat: NaN, lng: NaN };
        const lat = typeof loc.lat === 'function' ? loc.lat() : (loc as unknown as MapLatLng).lat;
        const lng = typeof loc.lng === 'function' ? loc.lng() : (loc as unknown as MapLatLng).lng;
        return { lat, lng };
      };
      ...
      if (isNaN(start.lat) || isNaN(start.lng) || isNaN(end.lat) || isNaN(end.lng) ||
        ...
        if (firstRoute.overview_path) {
          const coords = firstRoute.overview_path
            .map(p => {
              const lat = typeof p.lat === 'function' ? p.lat() : (p as unknown as MapLatLng).lat;
              const lng = typeof p.lng === 'function' ? p.lng() : (p as unknown as MapLatLng).lng;
        ...
        route.overview_path.forEach(point => {
          if (point) {
            ...
            if (typeof point.lat === 'function') {
              ...
              lat = (point as unknown as MapLatLng).lat;
              lng = (point as unknown as MapLatLng).lng;
            }
