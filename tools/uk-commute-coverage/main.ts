...
let analyzer: CoverageAnalyzer | undefined;
  ...
  previewBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (route.fullResult) {
        ...
        const optionsContainer = document.getElementById('route-options') as HTMLElement;
        ...
        if (route.fullResult.routes.length > 1) {
          route.fullResult.routes.forEach((r, index) => {
            ...
            btn.addEventListener('click', () => {
              ...
              const newCoordinates: Array<[number, number]> = selectedRoute.overview_path
                .map(latLng => {
          ...
          (document.getElementById('route-options-container') as HTMLElement).style.display = 'block';
        } else {
          (document.getElementById('route-options-container') as HTMLElement).style.display = 'none';
        }
      } else {
