...
test('Coordinate Converter - Multi-Zoom Support', async (t) => {
  ...
  await t.test('tile coordinates scale correctly between zoom 8 and zoom 10', async () => {
    ...
    assert.ok(ratioX > 3.5 && ratioX < 4.5,
      `X ratio ${ratioX} should be ~4 (z10: ${tileZ10.tileX}, z8: ${tileZ8.tileX})`);
    assert.ok(ratioY > 3.5 && ratioY < 4.5,
      `Y ratio ${ratioY} should be ~4 (z10: ${tileZ10.tileY}, z8: ${tileZ8.tileY})`);
  });
...
test('Coordinate Converter - Zoom 10 Reference Tests', async (t) => {
  ...
  await t.test('verified tile coordinates are correct at zoom 10', async () => {
    ...
    for (const verified of VERIFIED_TILES_Z10) {
      ...
      assert.strictEqual(recovered.tileX, verified.tileX,
        `Round-trip failed for ${verified.location} tileX`);
      assert.strictEqual(recovered.tileY, verified.tileY,
        `Round-trip failed for ${verified.location} tileY`);
    }
  });
  ...
  await t.test('pixel coordinates are within bounds at zoom 8', async () => {
    ...
    for (const loc of locations) {
      ...
      assert.ok(pixel.pixelX >= 0 && pixel.pixelX < 256,
        `Pixel X ${pixel.pixelX} out of bounds at zoom 8`);
      assert.ok(pixel.pixelY >= 0 && pixel.pixelY < 256,
        `Pixel Y ${pixel.pixelY} out of bounds at zoom 8`);
...
test('Coordinate Converter - DEFAULT_ZOOM uses STANDARD_ZOOM', async (t) => {
  await t.test('DEFAULT_ZOOM equals STANDARD_ZOOM', () => {
    assert.strictEqual(converter.DEFAULT_ZOOM, STANDARD_ZOOM,
      'DEFAULT_ZOOM should match STANDARD_ZOOM from constants');
  });
