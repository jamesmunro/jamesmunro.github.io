...
test('Coordinate Converter - Reverse Conversion', async (t) => {
    ...
    await t.test('pixel coordinates are within valid bounds', async () => {
        ...
        for (const loc of locations) {
            ...
            assert.ok(pixel.pixelX >= 0 && pixel.pixelX < 256, `Pixel X ${pixel.pixelX} out of bounds for ${loc.lat},${loc.lon}`);
            assert.ok(pixel.pixelY >= 0 && pixel.pixelY < 256, `Pixel Y ${pixel.pixelY} out of bounds for ${loc.lat},${loc.lon}`);
        }
    });
    await t.test('round-trip tile conversion preserves tile coordinates', async () => {
        ...
        for (const original of testTiles) {
            ...
            assert.strictEqual(recovered.tileX, original.tileX, `Round-trip failed for tileX ${original.tileX}`);
            assert.strictEqual(recovered.tileY, original.tileY, `Round-trip failed for tileY ${original.tileY}`);
        }
    });
});
